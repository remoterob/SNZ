...
import BingoPhotoExportAdmin from './pages/BingoPhotoExportAdmin'
import BingoRegistrationsAdmin from './pages/BingoRegistrationsAdmin'
...
function SNZHub() {
    ...
    supabase.from('bigfish_comps').select('id').eq('is_active', true).maybeSingle()
      .then(async ({ data: comp }) => {
        ...
        const [{ data: regs }, { data: ents }] = await Promise.all([
          supabase.from('bigfish_registrations').select('user_id').eq('comp_id', comp.id),
          supabase.from('bigfish_entries').select('user_id').eq('comp_id', comp.id),
        ])
        const ids = new Set([...(regs || []).map(r => r.user_id), ...(ents || []).map(e => e.user_id)])
        setBigFishCount(ids.size)
      })
  }, [])
  ...
  const modules = [
    ...
    },
    {
      ...
      status: 'live',
      summary: 'Registration open for 2027',
    },
    {
    ...
    },
    {
      ...
      status: 'sponsor',
      summary: 'Competition is live — log your catch now!',
    },
    {
...
export default function App() {
  return (
    ...
    <Routes>
      ...
      <Route path="/bingo/admin/photos"   element={<ProtectedBingoRoute><BingoPhotoExportAdmin /></ProtectedBingoRoute>} />
      <Route path="/bingo/admin/registrations" element={<ProtectedBingoRoute><BingoRegistrationsAdmin /></ProtectedBingoRoute>} />
      {/* AGM / SGM */}
